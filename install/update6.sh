...
if [ "${publicFileMd5}" != "${md5check}"  ]; then
	wget -O Tpublic.sh https://raw.githubusercontent.com/2duat/Panel/main/install/public.sh -T 20;
...
setup_path=/www
version=$(curl -Ss --connect-timeout 5 -m 2 https://raw.githubusercontent.com/2duat/Panel/main/api/panel/get_version)
if [ "$version" = '' ];then
	version='7.7.0'
fi
...
wget -T 5 -O /tmp/panel.zip https://raw.githubusercontent.com/2duat/Panel/main/install/src/LinuxPanel-7.7.0.zip
...
if [ "${check_bt}" = "" ];then
	rm -f /etc/init.d/bt
	wget -O /etc/init.d/bt https://raw.githubusercontent.com/2duat/Panel/main/install/src/bt6.init -T 20
